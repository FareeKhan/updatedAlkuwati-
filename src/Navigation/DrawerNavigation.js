...
const CustomDrawerContent = ({navigation}) => {
  ...
  console.log('activeSectionsactiveSections', activeSections);
    ...
    return (
      <View style={styles.content}>
        {Array.isArray(section?.sub) && section?.sub?.length > 0 ? (
          ...
          <Text
            style={{textAlign: 'center', paddingVertical: 20, color: '#000'}}>
            {t('NoCategories')}
          </Text>
        )}
      </View>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.menuContainer}>
        <ScrollView showsVerticalScrollIndicator={false}>
          <TouchableOpacity
            onPress={() =>
              navigation.navigate('StackNavigations', {screen: 'HomeScreen'})
            }
            activeOpacity={0.8}
            style={[
              styles.menuItem,
