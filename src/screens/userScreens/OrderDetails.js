...
const OrderDetails = ({navigation, route}) => {
  ...
  const calculateTotalWeight = () => {
    return data.reduce((total, product) => {
      const weight =
        product.productWeight == '00000' || product.productWeight == null
          ? 0.5
  ...
  const getDeliveryCharges = () => {
    ...
    const countryRules = deliveryChargesData.filter(
      rule => rule.country === updatCountry,
    );
    // Find matching range
  ...
  const FinalTotal =
    Number(totalDeliveryCharges) + Number(totalPrice) - Number(promoCodeValue);
    ...
    try {
      ...
      if (!result?.error) {
        const discountAmount =
          (FinalTotal * result?.promo_code?.discount) / 100;
      ...
      <View style={styles.productContainer}>
        ...
        <View style={{marginLeft: 10, width: '70%'}}>
          ...
          <View style={{flexDirection: 'row', alignItems: 'center', gap: 5}}>
            ...
            <CustomText style={{color: color.theme, fontWeight: '500'}}>
              x{item?.counter}
            </CustomText>
          </View>
        </View>
    ...
    <View style={styles.mainContainer}>
      ...
      <ScrollView
            ...
            <View style={styles.userAddressBox}>
              ...
              {userAddress?.country == t('Kuwait') ? (
                <>
                  <AddressLine
                    label={t('avenues')}
                    value={userAddress?.avenuePostal}
                  />
                  <AddressLine
                    label={t('governorate')}
        ...
        <View style={{flexDirection: 'row'}}>
          <Text style={[styles.productName, {fontSize: 15}]}>
            {data?.length > 1 ? t('product_items') : t('product_item')}
          </Text>
        </View>
        ...
        <View
          style={[styles.productContainer, {justifyContent: 'space-between'}]}>
