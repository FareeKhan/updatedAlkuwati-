...
const AllProductss = ({ navigation }) => {
    ...
    const OnBoxT = (id) => {
        //itemListAnimation
        if(innetCate == id){
            setItemListAnimation('fadeOutUp');
            setTimeout(() => {
                setInnetCate(0)    
              }, 1000)
        ...
        return (
            <>
                ...
                </Animatable.View>
                {
                    innetCate == item?.id &&
                    storeCat &&
                    <View style={{ marginTop: 0, marginBottom: 10, display: innetCate == item?.id ? 'flex' : 'none' }}>
